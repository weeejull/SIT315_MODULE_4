...
#define PRINT 0 // CONTROL VARIABLE FOR PRINTING ARRAY ELEMENTS

int SZ = 100000000; // DEFAULT SIZE OF THE ARRAYS
...
void setup_openCL_device_context_queue_kernel(char *filename, char *kernelname)
{
    ...
    context = clCreateContext(NULL, 1, &device_id, NULL, NULL, &err); // CREATE CONTEXT
    if (err < 0)
    {
    ...
    program = build_program(context, device_id, filename); // BUILD PROGRAM

    queue = clCreateCommandQueue(context, device_id, 0, &err); // CREATE COMMAND QUEUE
    if (err < 0)
    {
